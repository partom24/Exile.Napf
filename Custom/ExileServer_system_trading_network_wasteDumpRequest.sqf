...
try 
{
	...
	if (_mode isEqualTo 2) then
	{
		...
		_vehicleSellPrice = (_revenue - _revenue1);

		if (_vehicleSellPrice isEqualTo 0) then
		{
			_vehicle = ([(typeOf _vehicleObject)]);
			_message = format["Vehicle %1 sold for %2 :: %3", _vehicleObject, _vehicleSellPrice, _vehicle];
