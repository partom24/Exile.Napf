...
//if (!isServer || isDedicated) exitWith {"this isnt a server - custom" call ExileServer_util_log;};
...
if (_saveLoadOut && !isNull (unitBackpack _unit)) then {
	...
	[_unit,_class,_magazines,_weapons,_items] spawn {
		...
		"cleared backpakc items" call ExileServer_util_log;
		...
		};*/
		{
			if((_x select 0) || (_x select 1)) then
			{
				_item0 = _x select 0;
				_item1 = _x select 1;
				_unit addMagazineCargoGlobal[_item0,_item1];
			};
		} foreach _magazines;
		"returned mags to backpack" call ExileServer_util_log;
		{
			if((_x select 0) || (_x select 1)) then
			{
				_item0 = _x select 0;
				_item1 = _x select 1;
				_unit addWeaponCargoGlobal[_item0,_item1];
			};
		} foreach _weapons;
		...
		{	
			if((_x select 0) || (_x select 1)) then
			{
				_item0 = _x select 0;
				_item1 = _x select 1;
				_unit addItemCargoGlobal (_wep0);
			};
		} foreach _items;
...
//create a log entry
["Paradrop function has completed"] call ExileServer_util_log;
